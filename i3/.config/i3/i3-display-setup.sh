...
if [ $(xrandr | grep DP-2 | awk '{ print $2 }') = "connected" ]; then
    ...
    if [ $(xrandr | grep HDMI-1 | awk '{ print $2 }') = "connected" ]; then
        # home setup
        xrandr --output eDP-1 --mode 1920x1080 --pos 0x0 --rotate normal --output DP-1 --off --output HDMI-1 --primary --mode 1920x1080 --pos 1920x0 --rotate normal --output DP-2 --off
    else
